...
func findAllDoors(w http.ResponseWriter, r *http.Request) {
	...
	db.Raw(`select door from door where created > current_timestamp + interval - 15 second`).Scan(&doorList)
...
func findAllModes(w http.ResponseWriter, r *http.Request) {
	...
	// db.Table("door").Find(&doorList)
	db.Raw(`select mode from mode where created > current_timestamp + interval -15 second`).Scan(&modeList)
